...
const Card = ({ image, title, subtitle }: CardProps) => {
  return (
    <div
      className="w-full bg-[#252526] p-6 rounded-lg font-jost 
    ...
    >
      <img
        ...
        loading="lazy"
      />
      <h1 className="text-2xl font-bold pt-4 text-white tracking-wide">
        {title}
      </h1>
...
export default function Content() {
  return (
    <>
      ...
      <div id="Products" className="max-w-7xl m-auto font-jost pt-6 bg-background">
        {/* Title Section */}
        {/* <div className="text-center p-5 transition-all duration-1000">
          <h1 className="text-4xl md:text-8xl font-bold text-white border-b pb-5 border-gray-700">
            Products
          </h1>
        </div> */}
        <div className="text-left px-10 py-2 text-2xl">
          <h1>Photo Gallery</h1>
        </div>
        <div className="px-10">
          <ProductSwiper />
        </div>
        <div className="text-left px-10 pt-5 text-2xl">
          <h1>Produts</h1>
        </div>
        ...
        <div className="p-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-10">
          ...
          />
          <Card image={ETCHED} title="Etched Glass" subtitle="Texured Frost" />
          <Card
            image={STARGROOVE}
            title="Brown Mirror"
